...
class MY_Model extends CI_Model
{
    ...
    public function get_by()
    {
        ...
        $row = $this->_database->get($this->_table)->{$this->_return_type() }(0, $this->_entity_name);
        $this->_temporary_return_type = $this->return_type;
    ...
    public function get_all()
    {
        ...
        $result = $this->_database->get($this->_table)->{$this->_return_type(1) }($this->_entity_name);
        $this->_temporary_return_type = $this->return_type;
    ...
    public function fulltextsearchlimit($columget, $columindex, $again, $columnotin, $valuenotin, $limit)
    {
        if ($limit == 0) {
            if($valuenotin=='')
            {
                $sql = "SELECT " . $columget . " FROM " . $this->_table . " WHERE MATCH (" . $columindex . ") AGAINST ('" . $again . "' IN NATURAL LANGUAGE MODE)";
            }
            else
            {
                $sql = "SELECT " . $columget . " FROM " . $this->_table . " WHERE MATCH (" . $columindex . ") AGAINST ('" . $again . "' IN NATURAL LANGUAGE MODE) and " . $columnotin . " NOT IN (" . $valuenotin . ")";
            }
        } 
        else 
        {
            if($valuenotin=='')
            {
                $sql = "SELECT " . $columget . " FROM " . $this->_table . " WHERE MATCH (" . $columindex . ") AGAINST ('" . $again . "' IN NATURAL LANGUAGE MODE) LIMIT " . $limit;
            }
            else
            {
                $sql = "SELECT " . $columget . " FROM " . $this->_table . " WHERE MATCH (" . $columindex . ") AGAINST ('" . $again . "' IN NATURAL LANGUAGE MODE) and " . $columnotin . " NOT IN (" . $valuenotin . ") LIMIT " . $limit;
            }
        }
        return $query = $this->db->query($sql);
    }
    // public function fulltextsearch($columget, $columindex, $again,$columnotin,$valuenotin,$perpage, $offset)
    // {
    //     // $setindex = "ALTER TABLE ".$table." DROP INDEX ".$name.", ADD FULLTEXT INDEX (".$colum.")";
    //     // $this->db->query($setindex);
    //     $sql = "SELECT " . $columget . " FROM " . $this->_table . " WHERE MATCH (" . $columindex . ") AGAINST ('" . $again . "' IN NATURAL LANGUAGE MODE) and " . $columnotin . " NOT IN (" . $valuenotin . ") LIMIT " . $offset . ", " . $perpage;
    //     return $query = $this->db->query($sql);
    // }
    //     public function fulltextsearchnotlimit($columget, $columindex, $again,$columnotin,$valuenotin)
    // {
    //     $sql = "SELECT " . $columget . " FROM " . $this->_table . " WHERE MATCH (" . $columindex . ") AGAINST ('" . $again . "' IN NATURAL LANGUAGE MODE)";
